...
function Creative.Save(vehName)
	...
	if Passport then
        ...
        if vehicle[1] then
            ...
            exports.oxmysql:query_async("INSERT IGNORE INTO vehicles(Passport,vehicle,plate,work,rental,tax) VALUES(@Passport,@vehicle,@plate,@work,@rental,UNIX_TIMESTAMP() + 604800)",{ Passport = Passport, vehicle = vehName, plate = vRP.GeneratePlate(), rental = os.time() + 864000, work = "false" })
            if NotifyConfig then
                ...
                TriggerClientEvent("Notify",source,"verde","Veiculo "..vehName.." adicionado em sua garagem, Lembrando que a duração do mesmo é de 10 dias!!!",10000)
            end
        end
	end
end