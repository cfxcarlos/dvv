	...
	if Passport and args[1] then
		if vRP.HasGroup(Passport, "Admin") then
			if Prisoners[tonumber(args[1])] then
				TriggerClientEvent("police:Prisioner",vRP.Source(tonumber(args[1])),false)
                TriggerClientEvent("Notify",vRP.Source(tonumber(args[1])),"azul","Sua sentença terminou, esperamos não ve-lo novamente.",5000,"Sistema Penitenciário")
                exports.oxmysql:query_async("UPDATE characters SET prison = @prison WHERE id = @Passport",{ Passport = tonumber(args[1]), prison = parseInt(Prisoners[tonumber(args[1])]) })
...
AddEventHandler("Connect",function(Passport,source)
	local Consult = exports.oxmysql:query_async("SELECT * FROM characters WHERE id = @id",{ Passport = Passport })
	if Consult[1] and parseInt(Consult[1]["prison"]) > 0 then
		Wait(250)
		TriggerClientEvent("police:Prisioner",source,true)
		Prisoners[Passport] = parseInt(Consult[1]["prison"])
        TriggerClientEvent("Notify",source,"azul","Ainda vai passar <b>"..parseInt(Consult[1]["prison"]).." Meses</b> preso.",10000,"Sistema Penitenciário")
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- DISCONNECT
