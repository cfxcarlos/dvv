...
cRP = {}
Tunnel.bindInterface("nation_bennys", cRP)
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIÁVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
local nui = false
...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui and LocalPlayer["state"][bennys["permission"]] then
			idle = 1
			DrawMarker(27, bennys["pos"][1],bennys["pos"][2],bennys["pos"][3]-0.97 ,0,0,0,0,0,0,3.0,3.0,1.0,255, 102, 0,200,0,0,0,1)
			DrawMarker(36, bennys["pos"],0,0,0,0,0,0,1.0,3.0,1.0,255, 102, 0,200,0,0,0,1)
			local playercoords = GetEntityCoords(PlayerPedId())
			local distance = #(playercoords - bennys["pos"])
			if distance < 2 then
				...
				if IsControlJustPressed(0,38) then
					...
					if vehicle and func.checkVehicle(VehToNet(vehicle)) then
						...
						SetEntityHeading(vehicle,bennys["heading"])
...
function saveVehicle(vehicle)
	...
	vehicleMods["customPcolor"] = table.pack(GetVehicleCustomPrimaryColour(vehicle))
	vehicleMods["customScolor"] = table.pack(GetVehicleCustomSecondaryColour(vehicle))
	...
	func.saveVehicle(GetEntityArchetypeName(vehicle),GetVehicleNumberPlateText(vehicle),vehicleMods)
end
...
function getNearestBennys()
	...
	for i,j in ipairs(locais) do
		local distance = #(playercoords -j["pos"])
		if distance < 5 then
			return j
		end
	end
