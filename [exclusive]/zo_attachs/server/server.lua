...
Citizen.CreateThread(
    function()
        ...
        vRP.Prepare("zo/check_exists_table_attachs", "SHOW TABLES LIKE 'zo_attachs'")
        vRP.Prepare("zo/insert_attachs", "INSERT INTO zo_attachs(user_id, attachs) VALUES(@user_id, @attachs)")
        vRP.Prepare("zo/update_attachs", "UPDATE zo_attachs SET attachs = @attachs WHERE user_id = @user_id")
        vRP.Prepare("zo/get_attachs", "SELECT * FROM zo_attachs WHERE user_id = @user_id")
        async(
            function()
                if config.salvarAttachsDb then
                    vRP.Query("zo/attachs_createtable")
                end
            end
        )
    end
...
src.saveCacheWait = function()
    ...
    if next(cacheAttachsPlayersInWaitSave) then
        for user_id, atts in pairs(cacheAttachsPlayersInWaitSave) do
            if user_id then
                local attsUser = vRP.Query("zo/get_attachs", {user_id = parseInt(user_id)})
                if attsUser[1] ~= nil then
                    vRP.Query("zo/update_attachs", {user_id = parseInt(user_id), attachs = json.encode(atts)})
                else
                    vRP.Query("zo/insert_attachs", {user_id = parseInt(user_id), attachs = json.encode(atts)})
                end
...
src.get = function(puser_id)
    ...
    local atts = vRP.Query("zo/get_attachs", {user_id = parseInt(user_id)})
    if atts[1] ~= nil then
