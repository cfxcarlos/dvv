		...
		if not IsPedInAnyVehicle(Ped) then
			...
			for Number = 1, #SkinLocations do
				if #(Coords - SkinLocations[Number]) <= 2.0 then
					...
					if IsControlJustPressed(0, 38) and not exports["hud"]:Wanted() then
						OpenSkinshop()
					end
				end
			end
...
AddEventHandler("skinshop:Open", function(Status)
	Spawn = Status
...
RegisterNUICallback("Save", function(Data, Callback)
	...
	if Spawn then
		SetEntityCoordsNoOffset(PlayerPedId(),-1037.74,-2738.08,20.17,true,true,true)
		SetEntityHeading(PlayerPedId(),337.33)
