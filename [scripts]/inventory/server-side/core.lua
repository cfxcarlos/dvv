	...
	if Passport and not Active[Passport] and not exports["hud"]:Wanted(Passport)  and Amount > 0 then
		...
		if ClosestPed then
			...
			if OtherPassport then
				...
				if AmountAut ~= Amount then


					Amount = Amount - AmountAut
					local Identity = vRP.Identity(Passport)
					local Identity2 = vRP.Identity(OtherPassport)
					local webdrops = "/webhooks/1261937257971974175/3F6ZgSi-9pxA8crmF1bnhHDoMuv2D4LC_nK7Q-EpENACywVJm5qJIwzr_8y2SNAzSR07"
					SendWebhookMessage(webdrops, "```".. (Identity["name"].." "..Identity["name2"]) .." [".. Passport .."] enviou x"..Amount.."("..itemName(Item)..") para ".. (Identity2["name"].." "..Identity2["name2"]) .." [".. OtherPassport .."].```")
...
function Creative.Use(Slot,Amount)
	...
	if Passport and not Active[Passport] then
		...
		local Identity = vRP.Identity(Passport)
		local webdrops = "/webhooks/1261938189665570917/27MTqp1TVgEz6obaQM3icliVNpPeON-eVnTi-sppdILPw2FWajW6EmPNT_2CB-xMO3HK"
		SendWebhookMessage(webdrops, "```".. (Identity["name"].." "..Identity["name2"]) .." [".. Passport .."] usou x"..Amount.."("..itemName(Full)..") na cds ("..mathLength(Coords.x)..","..mathLength(Coords.y)..","..mathLength(Coords.z)..").```")
		...
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				...
				if ParkingMeters[Number]["Passport"] == Passport then
					...
					local Grupo = vRP.NumPermission("Policia")
