...
--[[ 	["drugtable"] = function(source,Passport,Amount,Slot,Full,Item,Split)
       -- if not exports['drugtable']:InValidLocation(source) then return end

        if vRP.TakeItem(Passport,Full,1,true,Slot) then
            exports['drugtable']:Create(source)
        end
    end, ]]
	["drugtable"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		for _,OtherSource in pairs(ClosestPeds) do
			if source ~= OtherSource then
				...
				for k,v in pairs(List) do
					local Rank = vRP.Groups()["Pcerj"]["Hierarchy"][v]
	...
	["lanca"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		TriggerClientEvent("inventory:Close",source)
		TriggerClientEvent("Progress",source,5000)
		vRP.createObjects(source,"amb@incar@male@smoking@enter","enter","mah_lanca", 48, 28422, 0.0, 0.01, 0.0, -10.0, -10.0, 0.0)
		repeat
			if Active[Passport] and os.time() >= parseInt(Active[Passport]) then
				vRPC.removeObjects(source)
				Active[Passport] = nil
	...
	["laptop"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		elseif vRP.HasPermission(Passport,"Bope") and vRP.HasService(Passport, "Bope") then
			permis = true
		elseif vRP.HasPermission(Passport,"PoliciaFederal") and vRP.HasService(Passport, "PoliciaFederal") then
			permis = true
