		...
		if not IsPedInAnyVehicle(Ped) and Drops[Route] then
			...
			for _,v in pairs(Drops[Route]) do
				if #(Coords - v["Coords"]) <= 50 then
					...
					DrawMarker(20,v["Coords"]["x"],v["Coords"]["y"],v["Coords"]["z"] + 0.5,0.0,0.0,0.0,0.0,180.0,0.0,0.25,0.35,0.25,255,0,0,500,1,1,1,1)
				end
			end
		end
...
RegisterNUICallback("Pickup",function(Data,Callback)
	if MumbleIsConnected() then
		vSERVER.Pickup(tostring(Data["id"]),tostring(Data["target"]),Data["amount"])
	end
	Callback("Ok")
end)
