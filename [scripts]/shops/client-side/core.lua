...
local List = {
	...
	{ 46.73,-1749.73,29.62,"Tools",false }, 
	...
	{ 484.37,-1161.74,30.75,"Paramedic",false },
	...
	{ 1566.08,1446.43,111.17,"Turano",false },
	{ 19.56,-1110.03,29.79,"Attachs",false,_,"Attachs" },
	{ 20.96,-1110.29,29.79,"Spray",false,_,"Spray" },
	...
	{ -1304.8,-297.77,40.73,"mecanica",false,_,"mecanica" }, 
	...
	-- Policia Civil
	{ -2105.16,-534.47,12.97,"Armory5",false,_,"Pcerj" },
	-- Policia Militar
	{ 922.55,-3185.69,6.72,"Armory5",false,_,"Pmerj" },
	-- Batalhão Bope 
	{ -251.69,1551.75,346.38,"Armory5",false,_,"Bope" },
    -- Delegacia Policia Federal
	{ 380.54,-739.27,29.96,"Armory5",false,_,"PoliciaFederal" },
	-- Batalhão Choque
	{ -578.28,-2393.77,19.12,"Armory5",false,_,"Choque" },
	-- Batalhão Recom
	{ -611.34,-2351.27,16.07,"Armory5",false,_,"Recom" },
	-- Batalhão Prf
	{ -308.14,6107.08,33.62,"Armory5",false,_,"Prf" },
	{ -1424.72,5133.08,62.66,"Armory5",false,_,"Prf" },
	-- EB
	{ -1753.51,3180.08,32.91,"Exercito",false,_,"Exercito" },
	-- Mecanica 
	{ -1288.14,-281.53,36.82,"Mecanica",false,_,"AutoSport" },
	{ -1296.36,-285.66,40.73,"Mec",false,_,"AutoSport" },
	{ -1294.54,-284.8,36.82,"Mecanica",false,_,"AutoSport" },
	{ -1301.51,-288.55,36.82,"Mecanica",false,_,"AutoSport" },
	{ -1308.14,-291.95,36.82,"Mecanica",false,_,"AutoSport" },
...
AddEventHandler("shops:Medic",function()
	if not exports["hud"]:Wanted() and vSERVER.Permission("Hospital") then
		SetNuiFocus(true,true)
...
-----------------------------------------------------------------------------------------------------------------------------------------
local Matutos = {
	{ 1742.41,-1637.47,112.52},
	{ 2475.87,-384.17,94.39},
	{ 2790.28,1460.58,24.48},
	{ 2885.41,4387.34,50.73},
	{ 44.05,6303.75,31.22},
	{ -567.38,5252.98,70.47},
	{ -1096.93,-1680.34,4.36},
	{ 501.86,-1966.0,24.99},
}
local Time = nil
local Matuto = nil
AddEventHandler("shops:Matuto",function()
	if not Time or GetGameTimer() >= Time then
		Time = GetGameTimer() + (20 * 60 * 1000)
		Matuto = math.random(#Matutos)
		SetNewWaypoint(Matutos[Matuto][1], Matutos[Matuto][2])
		TriggerEvent("Notify","amarelo","Matuto foi localizado, esta marcado no seu mapa.",5000)
		TriggerEvent("Notify","azul","Você tem 20min para entra em contato.",5000)
	end
end)

AddEventHandler("shops:MatutoOpen",function(Number)
	if Matuto and Matutos[Number] then
		SendNUIMessage({ Action = "Open", name = "Matuto", type = "Buy" })
		SetNuiFocus(true,true)
	end
end)

CreateThread(function()
	for Number,v in pairs(Matutos) do
		exports["target"]:AddCircleZone("Matutos:"..Number,vec3(v[1],v[2],v[3]),0.50,{
			name = "Matutos:"..Number,
			heading = 0.0
		},{
			shop = Number,
