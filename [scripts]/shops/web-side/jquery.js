	...
	$.post("http://shops/Request",JSON.stringify({ shop: selectShop }),(data) => {
		...
		for (let x = 1; x <= 25; x++){
			...
			if (data.inventoryUser[slot] !== undefined){
				...
				if (actualPercent <= 1)
					...
					const item = `<div class="item populated" title="" style="background-image: url('http://181.215.236.148/rj/inventory/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-stack="${v["stack"]}" data-item-key="${v["key"]}" data-slot="${slot}" data-peso="${v["peso"]}" data-description="${v["desc"]}">
					<div class="top">
		...
		for (let x = 1; x <= data["shopSlots"]; x++){
			...
			if (data.inventoryShop[slot] !== undefined){
				...
				const item = `<div class="item populated" title="" style="background-image: url('http://181.215.236.148/rj/inventory/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-item-key="${v["key"]}" data-slot="${slot}" data-price="${v["price"]}" data-peso="${v["peso"]}" data-description="${v["desc"]}" data-slot="${slot}">
					<div class="top">
						<div class="itemWeight">${(v["peso"]).toFixed(2)}</div>
						<div class="itemPrice">$${ParseFormat(v["price"])}</div>
					</div>
