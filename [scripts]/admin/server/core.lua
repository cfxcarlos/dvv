...
-- admin:DYNAMIC
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("admin1:Dynamic")
AddEventHandler("admin1:Dynamic", function(Mode)
	...
	if Passport then
		if Mode == "wl" then
			if vRP.HasGroup(Passport,"admin",1) then
				...
				if Keyboard then
					TriggerClientEvent("Notify",source,"sucesso","Whitelist editada.","Sucesso",5000)
		...
		elseif Mode == "rename" then
			if vRP.HasGroup(Passport,"admin",2) then
				...
				if Keyboard then
					...
					TriggerClientEvent("Notify",source,"sucesso","Nome atualizado.","Sucesso",5000)
			...
			if vRP.HasGroup(Passport,"admin") then
				...
				if Keyboard then
					...
					if vRP.Request(Passport, "Inventário", "Você realmente deseja limpar o inventário de <b>"..FullName.."</b>?") then
						...
						TriggerClientEvent("Notify",source,"sucesso","Limpeza concluída.","Sucesso",5000)
		...
		elseif Mode == "gem" then
			if vRP.HasGroup(Passport,"admin") then
				...
				if Keyboard then
					...
					if Identity then
						TriggerClientEvent("Notify",source,"sucesso","Gemas entregues.","Sucesso",5000)
			...
			if vRP.HasGroup(Passport,"admin") then
				...
				if Keyboard then
					if itemBody(Keyboard[1]) ~= nil then
						...
						TriggerClientEvent("Notify",source,"sucesso","Envio concluído.","Sucesso",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
			...
			if vRP.HasGroup(Passport,"admin",2) then
				...
				if Keyboard then
					...
					if vRP.Request(source,"Deletar Conta","Você tem certeza?") then
						...
						TriggerClientEvent("Notify",source,"sucesso","Personagem <b>"..OtherPassport.."</b> deletado.","Sucesso",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
			...
			if vRP.HasGroup(Passport,"admin",2) then
				...
				if Keyboard then
					...
					if ClosestPed then
						...
						TriggerClientEvent("Notify",source,"sucesso","Skin <b>"..Keyboard[2].."</b> setada no ID "..parseInt(Keyboard[1])..".","Sucesso",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
				...
				if Keyboard then
					...
					if ClosestPed then
						...
						if Identity then
							...
							TriggerClientEvent("Notify",source,"sucesso","Skin do ID "..parseInt(Keyboard[1]).." foi resetada.","Sucesso",5000)
						end
					end
				end
			else
		...
		elseif Mode == "kick" then
			if vRP.HasGroup(Passport,"admin",2) then
				...
				if Keyboard then
					...
					if OtherSource then
						TriggerClientEvent("Notify",source,"sucesso","Passaporte <b>"..Keyboard[1].."</b> expulso.","Sucesso",5000)
				...
				if Keyboard then
					...
					if Identity then
						...
						if OtherSource then
							...
							TriggerClientEvent("Notify",source,"aviso","Passaporte <b>"..OtherPassport.."</b> banido por <b>"..Days.."</b> dias.","Atenção",5000)
						end
					end
				end
			else
			...
			if vRP.HasGroup(Passport,"admin",2) then
				...
				if Keyboard then
					...
					if Identity then
						...
						exports["vrp"]:Embed("admin","**Passaporte:** "..Passport.."\n**Comando:** unban "..Keyboard[1],0xa3c846)
						TriggerClientEvent("Notify",source,"sucesso","Passaporte <b>"..OtherPassport.."</b> desbanido.","Sucesso",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
		...
		elseif Mode == "timeset" then
			if vRP.HasGroup(Passport,"admin",1) then
				...
				if Keyboard then
					...
					TriggerClientEvent("Notify",source,"aviso","Você mudou a hora do mundo.","Atenção",5000)
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
			end
			...
			if vRP.HasGroup(Passport,"admin",1) then
				...
				if Keyboard then
					if Keyboard[1] == "Sul" then
						...
						TriggerClientEvent("Notify",source,"aviso","Você mudou o clima do <b>Sul</b>.","Atenção",5000)
					elseif Keyboard[1] == "Norte" then
						GlobalState["WeatherN"] = Keyboard[2]
						TriggerClientEvent("Notify",source,"aviso","Você mudou o clima do <b>Norte</b>.","Atenção",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
			...
			if vRP.HasGroup(Passport,"admin",1) then
				...
				if Keyboard then
					if Keyboard[1] == "Sul" then
						...
						TriggerClientEvent("Notify",source,"aviso","Você mudou a temperatura do <b>Sul</b>.","Atenção",5000)
					elseif Keyboard[1] == "Norte" then
						GlobalState["TemperatureN"] = parseInt(Keyboard[2])
						TriggerClientEvent("Notify",source,"aviso","Você mudou a temperatura do <b>Norte</b>.","Atenção",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
		...
		elseif Mode == "blackoutset" then
			if vRP.HasGroup(Passport,"admin",1) then
				if GlobalState["Blackout"] then
					...
					TriggerClientEvent("Notify",source,"aviso","Modo blackout desativado.","Atenção",5000)
				else
					GlobalState["Blackout"] = true
					TriggerClientEvent("Notify",source,"sucesso","Modo blackout ativado.","Sucesso",5000)
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
			end
				...
				if Keyboard then
					...
					if vRP.GroupType(Permission) then
						if not vRP.GetUserType(OtherPassport,"Work") then
							...
							TriggerClientEvent("Notify",source,"sucesso","Adicionado <b>"..Permission.."</b> ao passaporte <b>"..OtherPassport.."</b>.","Sucesso",5000)
							vRP.SetPermission(OtherPassport,Permission,Level)
						else
							TriggerClientEvent("Notify",source,"aviso","O passaporte já pertence a outro grupo.","Atenção",5000)
						end
					else
						exports["vrp"]:Embed("admin","**Passaporte:** "..Passport.."\n**Comando:** group "..OtherPassport.." "..Permission.." "..Level,0xa3c846)
						TriggerClientEvent("Notify",source,"sucesso","Adicionado <b>"..Permission.."</b> ao passaporte <b>"..OtherPassport.."</b>.","Sucesso",5000)
		...
		elseif Mode == "ungroup" then
			if vRP.HasGroup(Passport,"admin",1) then
				...
				if Keyboard then
					TriggerClientEvent("Notify",source,"sucesso","Removido <b>"..Keyboard[2].."</b> ao passaporte <b>"..Keyboard[1].."</b>.","Sucesso",5000)
				...
				if not vRPC.InsideVehicle(source) then
					...
					if Vehicle then
						...
						if Keyboard then
							...
							TriggerClientEvent("Notify",source,"sucesso","Veículo com <b>"..parseInt(Keyboard[1]).."% de Gasolina</b>.","Sucesso",5000)
						end
					end
				else
					TriggerClientEvent("Notify",source,"aviso","Você precisa sair do veículo.","Atenção",5000)
				end
			else
				TriggerClientEvent("Notify",source,"aviso","Você não tem permissões para isso.","Atenção",5000)
			end
		...
		elseif Mode == "rembank" then
			if vRP.HasGroup(Passport,"admin") then
				...
				if Keyboard then
					...
					TriggerClientEvent("Notify",source,"sucesso","Remoção concluída.","Sucesso",5000)
		...
		elseif Mode == "setcar" then
			if vRP.HasGroup(Passport,"admin") then
				...
				if Keyboard then
					...
					if Consult[1] then
						TriggerClientEvent("Notify",source,"aviso","O veículo <b>"..Keyboard[2].."</b> já está adicionado.","Atenção",5000)
						return
					else
		...
		elseif Mode == "remcar" then
			if vRP.HasGroup(Passport,"admin") then
				...
				if Keyboard then
					...
					TriggerClientEvent("Notify",source,"sucesso","Veículo removido com sucesso.","Sucesso",5000)
		...
		elseif Mode == "changework" then
			if vRP.HasGroup(Passport,"admin",2) then
				...
				if Keyboard then
					...
					TriggerClientEvent("Notify",source,"sucesso","Emprego modificado.","Sucesso",5000)
...
RegisterCommand("shutdown",function(source)
	TriggerClientEvent("Notify",-1,"negado","Os geólogos informaram para nossa unidade governamental que foi encontrado um abalo de magnitude <b>60</b> na <b>Escala Richter</b>, encontrem abrigo até que o mesmo passe.","Terromoto",60000)
	...
	if Passport then
		if vRP.HasGroup(Passport,"admin") then
			if Spectate[Passport] then
				...
				TriggerClientEvent("Notify",source,"aviso","Modo espião desativado.","Atenção",5000)
			else
					...
					if OtherPassport and OtherIdentity then
						if vRP.Request(source,"administração","Você realmente deseja espionar <b>"..OtherIdentity["Name"].." "..OtherIdentity["Lastname"].."</b>?") then
							...
							if DoesEntityExist(Ped) then
								...
								TriggerClientEvent("Notify",source,"sucesso","Você está espiando <b>"..OtherIdentity["Name"].." "..OtherIdentity["Lastname"].."</b>.","Sucesso",5000)
							end
						end
					end
				end
